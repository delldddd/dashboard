...
def extract_merchant(text):
    ...
    # First try this pattern: after UPI- or UPI/, grab next part until next dash or @ or space
    m = re.search(r"UPI[-/](.+?)(?:-|@|\s|$)", s, re.IGNORECASE)
    if m:
        merchant = m.group(1).strip()
        # Avoid returning "UPI" itself
        if merchant.upper() != "UPI":
            return merchant
    ...
    # fallback: pick first chunk before comma, pipe, colon, or dash
    parts = re.split(r"[,|:-]", s)
    for part in parts:
        part = part.strip()
        if part.upper() != "UPI" and part != "":
            return part[:60]
    # fallback: return whole text truncated
    return s[:60]

# Robust file reader for CSV / XLSX / TXT uploads
def read_uploaded_file(uploaded_file):
    name = getattr(uploaded_file, "name", "uploaded").lower()
    ext = name.split(".")[-1] if "." in name else ""

    # CSV or TXT (including pipe/semicolon/tab separated)
    if ext in {"csv", "txt"} or ext == "":
        ...
        for enc in ("utf-8", "utf-8-sig", "cp1252", "latin1"):
            try:
                ...
                last_exc = e
        # Fallback: treat it as a simple line-based text file
        try:
            uploaded_file.seek(0)
            text = uploaded_file.read().decode("utf-8", errors="ignore")
            lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
            return pd.DataFrame({"Narration": lines})
        except Exception:
            raise ValueError(f"Unable to parse uploaded file as CSV/TXT: {last_exc}")
    ...
    if ext in {"xlsx", "xls"}:
        ...
        try:
            ...
            # Retry with dtype=str to be more forgiving
            uploaded_file.seek(0)
            try:
...
if uploaded is not None:
    ...
    for c in raw.columns:
        if re.search(r"amount|amt", c, re.IGNORECASE) and amount_col is None:
            amount_col = c
        if re.search(r"(debit|withdraw|withdrawal|dr)", c, re.IGNORECASE) and debit_col is None:
            debit_col = c
        if re.search(r"(credit|deposit|cr)", c, re.IGNORECASE) and credit_col is None:
            credit_col = c
