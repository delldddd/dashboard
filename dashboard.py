...
def extract_merchant(text):
    # VERSION: 4.0 - Using your approach but fixed to capture full merchant names
    print("🚨 EXTRACT_MERCHANT FUNCTION CALLED 🚨")
    if pd.isna(text):
        return ""
    raw_name = str(text)
    
    print(f"🚨 DEBUG: Processing text: {raw_name}")

    # If UPI format: extract between UPI- and @ (FULL MERCHANT NAME)
    upi_match = re.search(r'UPI-([^@]+)@', raw_name, re.IGNORECASE)
    if upi_match:
        merchant = upi_match.group(1).strip()
        print(f"🚨 DEBUG: UPI merchant extracted: {merchant}")
        return merchant

    # If FT format: extract everything after third dash
    if raw_name.upper().startswith("FT-"):
    ...
    # If no UPI/FT, return first 4 words as merchant
    words = raw_name.split()
    merchant = " ".join(words[:4])
    print(f"🚨 DEBUG: Fallback merchant extracted: {merchant}")
    return merchant
