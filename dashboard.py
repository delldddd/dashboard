...
print("🔧 TESTING EXTRACT_MERCHANT FUNCTION ON STARTUP 🔧")
print("🔧 THIS SHOULD APPEAR IMMEDIATELY WHEN THE FILE LOADS 🔧")
...
def extract_merchant(text):
    # VERSION: 3.0 - Complete rewrite with explicit patterns
    print("🚨 EXTRACT_MERCHANT FUNCTION CALLED 🚨")
    if pd.isna(text):
        return ""
    s = str(text)
    
    print(f"🚨 DEBUG: Processing text: {s}")

    # Pattern 1: UPI transactions - extract everything between UPI- and @
    if s.upper().startswith("UPI-") or s.upper().startswith("UPI/"):
        # Find the first @ symbol
        at_index = s.find("@")
        if at_index > 0:
            # Extract everything between UPI- and @
            merchant = s[4:at_index].strip()  # Skip "UPI-"
            if merchant and merchant.upper() != "UPI" and len(merchant) > 2:
                print(f"DEBUG: UPI merchant extracted: {merchant}")
                return merchant

    # Pattern 2: FT transactions - extract everything after third dash
    if s.upper().startswith("FT-"):
        parts = s.split("-")
        if len(parts) >= 4:
            merchant = "-".join(parts[3:]).strip()  # Everything after third dash
            if merchant and len(merchant) > 2:
                print(f"DEBUG: FT merchant extracted: {merchant}")
                return merchant

    # Pattern 3: REV-UPI transactions
    if s.upper().startswith("REV-UPI-"):
        at_index = s.find("@")
        if at_index > 0:
            # Find the merchant part after REV-UPI-REF-
            parts = s.split("-")
            if len(parts) >= 4:
                merchant = "-".join(parts[3:at_index]).strip()
                if merchant and merchant.upper() != "UPI" and len(merchant) > 2:
                    print(f"DEBUG: REV-UPI merchant extracted: {merchant}")
                    return merchant

    # Pattern 4: @upi format
    atupi_match = re.search(r"@upi[:\s]*([A-Za-z0-9._-]+)", s, re.IGNORECASE)
    if atupi_match:
        merchant = atupi_match.group(1).strip()
        print(f"DEBUG: @upi merchant extracted: {merchant}")
        return merchant

    # Pattern 5: "to:" or "from:" patterns
    to_match = re.search(r"to:?\s*([A-Za-z0-9 &._@-]{3,})", s, re.IGNORECASE)
    if to_match:
        merchant = to_match.group(1).strip()
        print(f"DEBUG: 'to' merchant extracted: {merchant}")
        return merchant

    from_match = re.search(r"from:?\s*([A-Za-z0-9 &._@-]{3,})", s, re.IGNORECASE)
    if from_match:
        merchant = from_match.group(1).strip()
        print(f"DEBUG: 'from' merchant extracted: {merchant}")
        return merchant

    # Fallback: return the whole text (truncated) without splitting on dashes
    print(f"DEBUG: Using fallback, returning: {s[:60]}")
    return s[:60]

# Test the function
test_result = extract_merchant("UPI-MRS LALITA SUNIL DUR-DURGOLI.LALITA-4@OKAXIS-CBIN0282611-545972301668-UPI")
...
st.error("🚨 DEBUG: This is the UPDATED version of the app! 🚨")
...
if uploaded is not None:
    st.success("🚨 FILE UPLOADED - DEBUGGING ENABLED 🚨")
    try:
    ...
    if narration_col:
        print(f"🔧 APPLYING EXTRACT_MERCHANT TO COLUMN: {narration_col}")
        print(f"🔧 SAMPLE DATA: {df[narration_col].head(3).tolist()}")
        df["Merchant"] = df[narration_col].apply(extract_merchant)
    else:
        # fallback: try to find meaningful text column
        text_cols = [c for c in df.columns if df[c].dtype == object]
        print(f"🔧 FALLBACK TEXT COLS: {text_cols}")
        if text_cols:
            print(f"🔧 APPLYING EXTRACT_MERCHANT TO COLUMN: {text_cols[0]}")
            print(f"🔧 SAMPLE DATA: {df[text_cols[0]].head(3).tolist()}")
